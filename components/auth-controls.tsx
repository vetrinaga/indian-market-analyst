...
export function AuthControls() {
  ...
  if (!email) return <a href="/auth/login" className="rounded-md border border-input px-3 py-2 text-xs font-semibold text-primary hover:bg-muted">Sign in</a>
  return <div className="flex items-center gap-2"><span className="hidden max-w-32 truncate text-[11px] text-muted-foreground sm:block">{email}</span><button onClick={async () => { await createClient().auth.signOut(); window.location.href = '/auth/login' }} className="rounded-md border border-input px-3 py-2 text-xs font-semibold text-primary hover:bg-muted">Sign out</button></div>
}
